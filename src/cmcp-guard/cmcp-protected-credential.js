...
import {cmcpProjectRoot, cmcpProjectProcessEnvironment} from './cmcp-project-paths.js';
/** Read only an explicit CMCP credential reference through a private captured pipe. */
export async function readCmcpProtectedCredential(referencePath, provider = "deepseek") {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(executable, ["-NoProfile", "-NonInteractive", "-File", script, "-Operation", "Read", "-Provider", provider, "-ReferencePath", referencePath],
      { cwd: cmcpProjectRoot, env: cmcpProjectProcessEnvironment(), windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
